...
(function(jq) {
    ...
    function gotoTop() {
        ...
        str += '</ul><div class="slide-help nav_top_list">';
        var cityCode = getCookie('townid', 1);
        if(isGroundCity(cityCode)){
            str+= '<a href="javascript:void(0);" class="ico-consult"><i></i><span>在线咨询</span></a>';
        }

        str += '<a target="_blank" href="http://www.to8to.com/about/feedback.php" rel="nofollow" class="ico-feedback"><i></i><span>意见反馈</span></a>'+
                        '<a href="javascript:;" class="ico-attention"><i></i><span>关注有礼</span></a>'+
                        '<a target="_blank" href="http://www.to8to.com/yezhu/zxbj.php?ptag=1_7_11_427" class="ico-quoted"></a>'+
                        '<a href="javascript:;" class="ico-backtop"><i></i><span>回到顶部</span></a>'+
                        '<div class="attention-code"><div class="attention-code-img"><img src="http://img.to8to.com/to8to_img/img_code/code_other.jpg" alt="" /><p>微信扫码</p><p>获取省钱秘笈</p></div></div>'+
                '</div></div>';

        jq("body").append(jq(str));
        obj = getBodyType();

        jq(".ico-attention").on('mouseover', function(){
            jq('.attention-code').stop().animate({
                'width': '146px',
                'right': '54px'
            },300);
        });
        jq(".ico-attention").on('mouseout', function(){
            jq('.attention-code').stop().animate({
                'width': '0',
                'right': '60px'
            },300);
        });
        jq('.ico-backtop').click(function(){
    ...
    function ctrlLeft() {
        ...
        if((obj.type == "wide" && w <= 1363) || (obj.type == "narrow" && w <= 1050)) {
            ...
            jq(".nav_top").css({"right": "30px","margin-right":0});    
        }   
    };
...
//判断是否是落地城市
function isGroundCity(id) {
    var GroundCityArr = [1130,1121,1672,1181,2827,1103,591,2920,1695,2929,1547,1196,1681,762,118,1,2298,317,2013,612,1891,2912,2933,2951,2962,2619,974];
    var cityLen = GroundCityArr.length;
