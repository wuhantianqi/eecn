...
//  })
    //手机验证码
    jq('.verification span:last-child').click(function(){
        jq.ajax({
                type: 'post', // 提交方式 get/post
                url: '/index.php?tenders-shortMessage.html', // 需要提交的 url
                data: {
                    'mobile': jq("input[name='data[mobile]']").val(),
                },
                dataType: "json",
                success: function(data) { 
                    if( data.error == 1 ){     //发送验证码成功
                        jq("#veritry").val( data.message[0]);  
                    }
                }
        });
    });
    ...
    function validData(){
        var chkArr = [
         //   {
         //     id: jq('.con_bj_cal .element :input[name="verification"]')[0],
