...
class Mdl_Helper_Link extends Model
{
    ...
    public function mklink($ctl, $args=array(), $params=array(), $http=false, $rewrite=true, $ext='.html')
    {
        ...
        /*
        if($_CFG['site']['domain']){
            if(in_array($ctl, array('case', 'article', 'home', 'ask'))){
                $ctl_domain = $_CFG['domain'][$ctl];
            }else if($ctl == 'mall/product'){
                $ctl_domain = $_CFG['domain']['product'];
            }else if(substr($ctl, 0, 5) == 'mall/'){
                $ctl_domain = $_CFG['domain']['product'];
            }else if(empty($act) && empty($args)){
                $route_ctl = '';
                $ext = '';
            }
            if($ctl_domain){
                $domain = $_CFG['site']['domain'];
                if(preg_match("/(\w+)\.{$domain}/i", $ctl_domain)){
                    $ctl_domain = null;
                }
            }
        }*/
        ...
        if($http){
            ...
            if($_CFG['site']['multi_city'] && $http && is_numeric($http)){
                if($city_id = (int)$http){
