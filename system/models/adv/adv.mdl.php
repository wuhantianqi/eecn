    ...
    public function block($params, $tmpl, $smarty)
    {
        ...
        if($items = $detail['items']){
            ...
            if(!in_array($adv['from'], array('photo', 'product', 'lunzhuan','falsh'))){
                if($adv['width']){
                    $adv_item_attr .= ' width="'.$adv['width'].'"';
                }
                if($adv['height']){
                    $adv_item_attr .= ' height="'.$adv['height'].'"';
                }
            }
            $item_list = array();
            foreach($items as $k=>$v){
            ...
            if(empty($tmpl)){
				...
				{
					$content .= str_replace(array('<{$item.a_attr}>','<{$pager.img}>','<{$item.clickurl}>','<{$item.title}>','<{$item.thumb}>','<{$item.item_attr}>'), array('target='."'".$item['target']."'",$cfg['url'],$item['link'],$item['title'],$item['thumb'],''), $match[1]);
				}
				if($match2){
