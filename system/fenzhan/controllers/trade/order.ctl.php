...
class Ctl_Trade_Order extends Ctl
{
    ...
    public function index($page=1)
    {
        ...
        $this->tmpl = 'fenzhan:trade/order/items.html';
    }

    public function so()
    {
        $this->tmpl = 'fenzhan:trade/order/so.html';
    }
    ...
    public function detail($order_id = null)
    {
        if(!$order_id = (int)$order_id){
            ...
            $this->tmpl = 'fenzhan:trade/order/detail.html';
        }
    }
    ...
    public function edit($order_id=null)
    {
        if(!($order_id = (int)$order_id) && !($order_id = $this->GP('order_id'))){
        	...
        	$this->pagedata['detail'] = $detail;
        	$this->tmpl = 'fenzhan:trade/order/edit.html';
        }
    }
    ...
    {
		if($order_id = (int)$order_id){
            if($shop = K::M('trade/order')->detail($order_id)){
                if($shop['city_id'] != CITY_ID){
                    $this->err->add('不可越权操作', 403);
                }else if(K::M('trade/order')->delete($order_id)){
                    $this->err->add('删除成功');
                }
            }
        }else if($ids = $this->GP('order_id')){
            if($items = K::M('trade/order')->items_by_ids($ids)){
                $aids  = array();
                foreach($items as $v){
                    if(CITY_ID && CITY_ID != $v['city_id']){
                        continue;
                    }
                    $aids[$v['order_id']] = $v['order_id'];
                }
                if($aids && K::M('trade/order')->delete($aids)){
                    $this->err->add('批量删除成功');
                }
            }
        }else{
