...
class Ctl_Member_Verify extends Ctl
{
    ...
    public function index($page=1)
    {
        ...
        if($items = K::M('member/verify')->items_by_city($filter, null, $page, $limit, $count)){
        	$pager['count'] = $count;
            $uids = array();
    ...
    public function edit($uid=null)
    {
        if(!($uid = (int)$uid) && !($uid = $this->GP('uid'))){
            ...
            $this->err->add('您要修改的用户不存在或已经删除', 212);
        }else if($member['city_id'] != CITY_ID){
            $this->err->add('您没有权限修改该用户', 212);
    ...
    public function dopass($uid=null)
    {
        if($uid = (int)$uid){
            ...
            if(K::M('member/verify')->update_verify($uids, 'pass')){
