...
class Ctl_Tools_Database extends Ctl
{
    ...
    public function index()
    {
        if($this->checksubmit()){
            ...
            $this->tmpl = 'fenzhan:tools/database/index.html';
        }
    }
    ...
    public function backup()
    {
        if($this->checksubmit()){
            if(!$tables = $this->GP('tables')){
                ...
                $this->cache->set('backuptables', $tables);
            ...
            $tables = $this->cache->get('backuptables');
            if(empty($tables)){
                ...
                $bakupfile = $bakupdir."/{$hash}_{$volumn}.sql";
    ...
    public function backlist()
    {
            ...
            if(is_dir($backupdir.$name)){
                if(preg_match('/^\d{8}_\w{8}$/i', $name)){
                    ...
                    while (false !== ($volumn = $fp->read())) {                        
                        if(preg_match("/^{$name}_\d+\.sql$/i", $volumn)){
        ...
        $this->tmpl = 'fenzhan:tools/database/backlist.html';
    }
    ...
    public function optimize()
    {
        if($this->checksubmit('tables')){
            ...
            $this->tmpl = 'fenzhan:tools/database/optimize.html';
        }
    }
