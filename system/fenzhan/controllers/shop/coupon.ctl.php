...
class Ctl_Shop_Coupon extends Ctl
{
    ...
    public function index($page=1)
    {
        ...
        $this->tmpl = 'fenzhan:shop/coupon/items.html';
    }

    public function so()
    {
        $this->tmpl = 'fenzhan:shop/coupon/so.html';
    }
    ...
    public function edit($coupon_id=null)
    {
        if(!($coupon_id = (int)$coupon_id) && !($coupon_id = $this->GP('coupon_id'))){
            ...
            $this->tmpl = 'fenzhan:shop/coupon/edit.html';
        }
    }
    ...
    {
        $this->tmpl = 'fenzhan:shop/coupon/download/so.html';
    }
    ...
    public function downloads($page=1)
    {
        ...
        $this->pagedata['items'] = $items;
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'fenzhan:shop/coupon/download/items.html';
    }
    ...
    public function downloadCoupon($coupon_id=null)
    {
        if(!($coupon_id = (int)$coupon_id) && !($coupon_id = $this->GP('coupon_id'))){
            ...
            $pager['page'] = max(intval($page), 1);
            $pager['limit'] = $limit = 50;
            if($items = K::M('shop/couponDownload')->items_by_coupon($coupon_id,$filter, $page, $limit, $count)){
                $pager['count'] = $count;
                $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink(null, array('{page}')), array('SO'=>$SO));
                $this->pagedata['items'] = $items;
            }
            ...
            $this->pagedata['pager'] = $pager;
            $this->tmpl = 'fenzhan:shop/coupon/download/coupon.html';
        }
    }
    ...
    public function downloadEdit($download_id=null)
    {
        if(!($download_id = (int)$download_id) && !($download_id = $this->GP('download_id'))){
            ...
            $this->pagedata['detail'] = $detail;
            $this->tmpl = 'fenzhan:shop/coupon/download/edit.html';
        }
    }
